    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
        {
            ...
            else
            {
                ...
                {
                    sWorld.SendWorldText(LANG_BG_STARTED_ANNOUNCE_WORLD, GetName(), GetInvitedCount(ALLIANCE), GetInvitedCount(HORDE));
                }
            }
        }
    }
...
void BattleGround::EndBattleGround(Team winner)
{
    ...
    if (isArena() && isRated())
    {
        ...
        if (winner_arena_team && loser_arena_team)
        {
            ...
            if(isPossibleWintrade())
                sLog.outInterest("Arena match Type: %u for Team1Id: %u - Team2Id: %u ended. WinnerTeamId: %u. RatingChange: %i.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE], winner_arena_team->GetId(), winner_change);
        }
        else
        {
...
void BattleGround::StartBattleGround()
{
    ...
    if(isPossibleWintrade())
        sLog.outInterest("Arena match type: %u for Team1Id: %u - Team2Id: %u started.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE]);
}
