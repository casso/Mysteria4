...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    if(plr)
    {
        if(HasFlag(CHANNEL_FLAG_LFG) &&
            sWorld.getConfig(CONFIG_BOOL_RESTRICTED_LFG_CHANNEL) && plr->GetSession()->GetSecurity() <= SECURITY_VIP &&
    ...
    if(m_announce && (!plr || plr->GetSession()->GetSecurity() < SECURITY_GAMEMASTER || !sWorld.getConfig(CONFIG_BOOL_SILENTLY_GM_JOIN_TO_CHANNEL) ))
    {
        MakeJoined(&data, p);
        SendToAll(&data);
    }
...
void Channel::Leave(uint64 p, bool send)
{
    ...
    else
    {
        ...
        players.erase(p);
        if(m_announce && (!plr || plr->GetSession()->GetSecurity() < SECURITY_MODERATOR || !sWorld.getConfig(CONFIG_BOOL_SILENTLY_GM_JOIN_TO_CHANNEL) ))
        {
            WorldPacket data;
            MakeLeft(&data, p);
            SendToAll(&data);
...
{
    AccountTypes sec = SECURITY_PLAYER;
    ...
    else if(!players[good].IsModerator() && sec < SECURITY_GAMEMASTER)
    {
        WorldPacket data;
    ...
    else
    {
        ...
        else if(sec < SECURITY_GAMEMASTER && bad->GetGUID() == m_ownerGUID && good != m_ownerGUID)
        {
            WorldPacket data;
            MakeNotOwner(&data);
            SendToOne(&data, good);
...
void Channel::Password(uint64 p, const char *pass)
{
    ...
    else if(!players[p].IsModerator() && sec < SECURITY_GAMEMASTER)
    {
        WorldPacket data;
        MakeNotModerator(&data);
        SendToOne(&data, p);
...
void Channel::SetMode(uint64 p, const char *p2n, bool mod, bool set)
{
    ...
    else
    {
        ...
        if( (plr->GetSession()->GetSecurity() < SECURITY_GAMEMASTER || newp->GetSession()->GetSecurity() < SECURITY_GAMEMASTER) &&
...
void Channel::SetOwner(uint64 p, const char *newname)
{
    ...
    if(sec < SECURITY_GAMEMASTER && p != m_ownerGUID)
    {
        WorldPacket data;
        MakeNotOwner(&data);
        SendToOne(&data, p);
...
void Channel::List(Player* player)
{
    ...
    else
    {
        ...
        for(PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
        {
            ...
            if (plr && (player->GetSession()->GetSecurity() > SECURITY_VIP || plr->GetSession()->GetSecurity() <= gmLevelInWhoList) &&
...
void Channel::Say(uint64 p, const char *what, uint32 lang)
{
    ...
    else if(m_moderate && !players[p].IsModerator() && sec < SECURITY_GAMEMASTER)
    {
        WorldPacket data;
        MakeNotModerator(&data);
        SendToOne(&data, p);
