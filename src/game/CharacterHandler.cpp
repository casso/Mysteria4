...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
    ...
    if (GetSecurity() <= SECURITY_VIP && sObjectMgr.IsReservedName(name))
    {
    ...
    bool AllowTwoSideAccounts = !sWorld.IsPvPRealm() || sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_ACCOUNTS) || GetSecurity() > SECURITY_PLAYER;
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recv_data)
{
    ...
    // check name limitations
    if (GetSecurity() <= SECURITY_VIP && sObjectMgr.IsReservedName(newname))
    {
        WorldPacket data(SMSG_CHAR_RENAME, 1);
        data << uint8(CHAR_NAME_RESERVED);
        SendPacket( &data );
