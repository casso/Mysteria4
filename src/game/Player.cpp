...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_VMAP_INDOOR_CHECK) && !isOutdoor && !IsFreeFlying())
...
void Player::SaveToDB()
{
    ...
    ss << "INSERT INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, exploredZones, equipmentCache, ammoId, knownTitles, actionBars) VALUES ("
        << GetGUIDLow() << ", "
    ...
    for(AuraMap::const_iterator itr = auras.begin(); ; ++itr)
    {
        if(itr == auras.end() || lastEffectPair != itr->first)
        {
            ...
            if (!itr2->second->IsPassive() && (itr2->second->GetCasterGUID() == GetGUID() || !itr2->second->IsSingleTarget()))
            {
                if (first_round)
                {
                    ss << "INSERT INTO character_aura (guid,caster_guid, item_guid, spell, effect_index, stackcount, amount, maxduration, remaintime, remaincharges)VALUES ";
                    first_round = false;
                }
                // next new/changed record prefix
                else
                ...
                ss << "("<< GetGUIDLow() << "," << itr2->second->GetCasterGUID() << "," << GUID_LOPART(itr2->second->GetCastItemGUID()) << ","
...
void Player::UpdateSpeakTime()
{
    ...
    if(m_speakTime > current)
    {
        ...
        if(m_speakCount >= max_count)
        {
            ...
            sWorld.SendGMWorldText(SECURITY_MODERATOR, LANG_ANTIFLOOD_NOTIFY, GetName(), sWorld.getConfig(CONFIG_UINT32_CHATFLOOD_MUTE_TIME));
