    ...
    npc_felmyst_vaporAI(Creature* pCreature) : ScriptedAI(pCreature)
   {
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!m_creature->getVictim())
        {
            ...
            Unit* pTarget =  m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0);  // maybe we need to exclude pets?
        ...
        if (Creature* pFelmyst = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_FELMYST)))//m_pInstance->GetSingleCreatureFromStorage(NPC_FELMYST))
