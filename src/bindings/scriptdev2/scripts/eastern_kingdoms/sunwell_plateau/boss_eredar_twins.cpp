...
{
    {SAY_INTRO_1, DATA_SACROLASH, 1000},
    {SAY_INTRO_2, DATA_ALYTHESS,  1500},
    {SAY_INTRO_3, DATA_SACROLASH, 1500},
    {SAY_INTRO_4, DATA_ALYTHESS,  1500},
    {SAY_INTRO_5, DATA_SACROLASH, 1500},
    {SAY_INTRO_6, DATA_ALYTHESS,  1500},
    {SAY_INTRO_7, DATA_SACROLASH, 2500},
    {SAY_INTRO_8, DATA_ALYTHESS,  0},
    {0, 0, 0},
};
...
struct MANGOS_DLL_DECL boss_alythessAI : public ScriptedAI
{
    ...
    void JustReachedHome()
    {
        if (m_pInstance)
        {
            ...
            // Respawn dead sister
            if (Creature* pSister = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_SACROLASH)))
            {
                if (!pSister->isAlive())
                    pSister->Respawn();
            }
    ...
    void JustDied(Unit* pKiller)
    {
        if (m_pInstance)
        {
            if (Creature* pSacrolash =  m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_SACROLASH)))
            {
...
struct MANGOS_DLL_DECL boss_sacrolashAI : public ScriptedAI
{
    ...
    void JustReachedHome()
    {
        if (m_pInstance)
        {
            ...
            // Respawn dead sister
            if (Creature* pSister = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_ALYTHESS)))
            {
                if (!pSister->isAlive())
                    pSister->Respawn();
            }
    ...
    void JustDied(Unit* pKiller)
    {
        if (m_pInstance)
        {
            if (Creature* pAlythess = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_ALYTHESS)))
            {
